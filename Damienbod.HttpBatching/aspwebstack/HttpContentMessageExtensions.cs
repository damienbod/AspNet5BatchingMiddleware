...
namespace Microsoft.Aspwebstack
{
    /// <summary>
    ...
    public static class HttpContentMessageExtensions
    {
            ...
            int bufferSize, int maxHeaderSize, CancellationToken cancellationToken)
        {
            ...
            {
                throw Error.Argument("uriScheme", "HttpMessageParserInvalidUriScheme", uriScheme, typeof(Uri).Name);
            }
            ...
            string uriScheme, int bufferSize, int maxHeaderSize, CancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                ...
                {
                    throw Error.InvalidOperation("HttpMessageParserError", headerConsumed, buffer);
                }
                else if (bytesRead == 0)
                {
                    throw new IOException("ReadAsHttpMessageUnexpectedTermination");
                }
            }
        }
        ...
        private static Uri CreateRequestUri(string uriScheme, HttpUnsortedRequest httpRequest)
        {
            ...
            if (httpRequest.HttpHeaders.TryGetValues(FormattingUtilities.HttpHostHeader, out hostValues))
            {
                ...
                if (hostCount != 1)
                {
                    throw Error.InvalidOperation("HttpMessageParserInvalidHostCount", FormattingUtilities.HttpHostHeader, hostCount);
                }
            }
            else
            {
                throw Error.InvalidOperation("HttpMessageParserInvalidHostCount", FormattingUtilities.HttpHostHeader, 0);
            }
        ...
        private static HttpContent CreateHeaderFields(HttpHeaders source, HttpHeaders destination, Stream contentStream, int rewind)
        {
            ...
            if (contentHeaders != null)
            {
                ...
                {
                    throw Error.InvalidOperation("HttpMessageContentStreamMustBeSeekable", "ContentReadStream", FormattingUtilities.HttpResponseMessageType.Name);
                }
